...
import { Award, Printer, X, Trophy, BookOpen, Lock, ShieldCheck, Calendar, Clock, LogIn, Globe, Sparkles } from "lucide-react";
import ExamReviewSheet from "@/components/exam/ExamReviewSheet";
import CertificateCard from "@/components/exam/CertificateCard";
...
export default function ExamResultModal({ resultData, onClose }: ExamResultModalProps) {
  ...
  const [viewMode, setViewMode] = useState<"score" | "certificate">("score");
      ...
      {viewMode === "certificate" ? (
        <div style={{ maxWidth: "880px", width: "100%" }}>
          <CertificateCard
            ...
            onClose={() => setViewMode("score")}
          />
        </div>
      ) : (
        <div className="q-card" style={{ maxWidth: "540px", width: "100%", padding: "1.4rem 1.6rem", position: "relative" }}>
          <button
            onClick={onClose}
            style={{
              position: "absolute",
          ...
          {/* TAB CHUYỂN ĐỔI BẢNG ĐIỂM & CHỨNG NHẬN NẾU THI ĐẠT */}
          {isPassed && (
            <div style={{
              display: "flex",
              ...
              >
                <Trophy size={14} />
                <span>Giấy Chứng Nhận Kết Quả 🏆</span>
              </button>
            </div>
          )}
          ...
          {/* Nút xem Chứng nhận nếu đạt chuẩn */}
          {isPassed && (
            <button
              onClick={() => setViewMode("certificate")}
              style={{
            ...
            >
              ...
              <span>📜 XEM & IN GIẤY CHỨNG NHẬN KẾT QUẢ THI</span>
            </button>
          )}
          ...
          {allowReview ? (
            ...
            >
              <BookOpen size={16} />
              <span>🔍 Xem Chi Tiết Đúng / Sai & Chạy Thử Code</span>
            </button>
          ) : (
            <div style={{
